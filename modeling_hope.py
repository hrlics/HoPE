# coding=utf-8
# Modification Copyright 2025 Haoran Li
...
class Qwen2VLRotaryEmbedding(nn.Module):
    def __init__(
        ...
        self.original_inv_freq = self.inv_freq # self.inv_freq.shape: [head_dim//2] (64)
...
# q.shape: torch.Size([2, 28, 7256, 128]) <==> (btz, num_head, seq_len, head_dim)
# cos.shape: torch.Size([3, 2, 7256, 128]) <==> ([t,x,y], btz, seq_len, head_dim)
# freqs: torch.Size([3, 2, 7256, 64]) <==> ([t,x,y], btz, seq_len, head_dim//2)


# MODIFIED
# Hybrid Frequency Allocation (HFA) of HoPE
def apply_hybrid_multimodal_rotary_pos_emb(q, k, cos, sin, mrope_section, unsqueeze_dim=1):
    ...
    for i, section in enumerate(mrope_section):
        if i % 2 == 0: # spatial
            for j in range(section):
                ...
                index += 1
        else: # temporal -> use nope
            result_cos.append(torch.ones_like(cos[0, ..., index:index + section]))
            result_sin.append(torch.zeros_like(cos[0, ..., index:index + section]))
            index += section
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    # MODIFIED
    # Dynamic Temporal Scaling (DTS) of HoPE
    def get_dynamic_rope_index(
        self,
        ...
        if image_grid_thw is not None or video_grid_thw is not None:
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(image_nums + video_nums):
                    ...
                    # MODIFIED
                    # dynamic scaling
                    scalor = random.choice(scale_factors)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if position_ids is None and input_ids is not None:                
                # MODIFIED
                # applly different multimodal RoPEs during training
                ...
                elif 'hope' in self.which_rope:
                    apply_multimodal_rotary_pos_emb = apply_hybrid_multimodal_rotary_pos_emb

                if self.which_rope == 'm_rope':
                    position_ids, _ = self.get_rope_index(
                        input_ids, image_grid_thw, video_grid_thw, attention_mask
                    )   
                elif self.which_rope == 'hope':
                    scale_factors = [0.5, 0.75, 1.0, 1.25, 1.5]
                    position_ids, _ = self.get_dynamic_rope_index(
                        input_ids, image_grid_thw, video_grid_thw, attention_mask, scale_factors=scale_factors
                    )
        ...
        if attention_mask is not None and position_ids is None:
            if cache_position is None or (cache_position is not None and cache_position[0] == 0):
                ...
                if 'videorope' in which_rope:
                    apply_multimodal_rotary_pos_emb = apply_m_modify_multimodal_rotary_pos_emb
                if 'hope' in which_rope:
                    apply_multimodal_rotary_pos_emb = apply_hybrid_multimodal_rotary_pos_emb
                
                # position_ids during inference
                if which_rope == 'm_rope':
                    position_ids, rope_deltas = self.get_rope_index(
                        input_ids, image_grid_thw, video_grid_thw, attention_mask
                    )
                elif which_rope == 'hope':
                    scale_factor = [-1]
                    assert scale_factor[0] > 0, "During inference of HoPE, scale_factor must be selected by user. Please modify scale_factor to a positive value."
                    position_ids, _ = self.get_dynamic_rope_index(
                        input_ids, image_grid_thw, video_grid_thw, attention_mask, scale_factors=scale_factor
                    )
